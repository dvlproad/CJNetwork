...
@implementation UIView (CJBlankHost)

#pragma mark - runtime
#pragma mark - 弹出到popupSuperview的视图
// cjBlankViews
- (NSMutableArray<UIView *> *)cjBlankViews {
    return objc_getAssociatedObject(self, @selector(cjBlankViews));
}

- (void)setCjBlankViews:(NSMutableArray<UIView *> *)cjBlankViews {
    objc_setAssociatedObject(self, @selector(cjBlankViews), cjBlankViews, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (NSInteger)showPopupContainer:(UIView *)popupContainer {
    ...
    if (self.cjBlankViews == nil) {
        self.cjBlankViews = [[NSMutableArray alloc] init];
    }
    [self.cjBlankViews addObject:popupContainer];
    
    NSInteger index = [self.cjBlankViews indexOfObject:popupContainer];
    return index;
}
...
- (void)hidePopupContainer:(UIView *)popupContainer {
    ...
    if (self.cjBlankViews != nil) {
        [self.cjBlankViews removeObject:popupContainer];
    }
}
...
- (void)__showOrHidePopupViews:(BOOL)show {
    NSArray *subViews2 = self.cjBlankViews;
