...
- (void)layoutSubviews {
    ...
    [self __addPopupViewCorner];
}

- (void)__addPopupViewCorner {
    /* 圆角化（注:如果圆角的区域刚好等于被进行模糊的区域，则模糊的区域也要圆角） */
    if (self.cornerViewPart == CQPanlinePopupViewPartAll) {
        [CQEffectAndCornerHelper cornerByMaskToView:self withCornerRadius:self.cornerRadius];
        
    } else if (self.cornerViewPart == CQPanlinePopupViewPartWithoutPanLine) {
        [CQEffectAndCornerHelper cornerByMaskToView:self.customView withCornerRadius:self.cornerRadius];
        // 如果模糊化操作有新视图生成，则再当要圆角化的子视图还等于模糊化的子视图时候，也要为此新模糊视图添加corner
        if (self.effectView != nil && self.effectViewPart == self.cornerViewPart) {
            [CQEffectAndCornerHelper cornerByMaskToView:self.effectView withCornerRadius:self.cornerRadius];
        }
    }
}

- (void)__removePopupViewCorner {
    if (_cornerViewPart == CQPanlinePopupViewPartAll) {
        self.layer.mask = nil;
    } else if (_cornerViewPart == CQPanlinePopupViewPartWithoutPanLine) {
        self.customView.layer.mask = nil;
        if (_effectView != nil && _effectViewPart == _cornerViewPart) {
...
{
    // 如果要做切换必须得先清除旧的（且注意：blankView有自己的effectView 、popupView 也有自己的effectView）
    [self __removePopupViewEffect];
...
- (void)cornerWithViewPart:(CQPanlinePopupViewPart)cornerViewPart cornerRadius:(CGFloat)cornerRadius {
    // 如果要做切换必须得先清除旧的（且注意：blankView有自己的corner,但没有需要设置的场景 、popupView 也有自己的corner）
    [self __removePopupViewCorner];
    
    _cornerViewPart = cornerViewPart;
    _cornerRadius = cornerRadius;
    
    if (cornerViewPart != CQPanlinePopupViewPartNone) {
        [self __addPopupViewCorner];
        [self setNeedsLayout];
    }
}
