...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:@"contentSize"] || [keyPath isEqualToString:@"frame"]) {
        [self cjTextView_adjustedContentInsetToTextCenter:0]; // 内部已使用layoutIfNeeded，不需要延迟
    }
}
...
- (void)cjTextView_adjustedContentInsetToTextCenter:(CGFloat)delay {
    [self cj_adjustedContentInsetToTextCenter:delay verticalAlignment:self.verticalAlignment];
    [self.placeholderView cj_adjustedContentInsetToTextCenter:delay verticalAlignment:self.verticalAlignment];
